...
# @Last Modified time: 2017-04-09 20:48:23
#
...
darkMatter = True
shape = 1							#0 : gaussian, 1: ellipse, 2: cross
...
def init_pos():
	...
	if shape == 0:							#gaussian
		...
		x1 = np.random.normal(0.,RD/2,N/2)		
		y1 = np.random.normal(0.,RD,N/2)
		x2 = np.random.normal(0.,RD,N/2)
		y2 = np.random.normal(0.,RD/2,N/2)
		x = np.append(x1,x2)
		y = np.append(y1,y2)
	return x,y
...
def show_fig417(x,y,i,xpos,ypos):
	...
	plt.savefig("testfig/417_%05d.png"%i,format='png')
	plt.close()
...
def show_fig421(vrot0,vrot1,i=None):
	...
	if i != None:
		plt.savefig("testfig/421_%d.png"%(i),format='png')
	else:
		plt.savefig("testfig/421.png",format='png', dpi=1000)	#last one in high quality
	plt.close()
